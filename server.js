...
/* ── 1) DB — اتصال متوافق مع بيئة Vercel السيرفرلس ──── */
...
const GameSchema = new mongoose.Schema({
  ...
  source:   { type: String, enum: ['local', 'rawg', 'itch'], default: 'local' },
  storeUrl: String, // رابط الصفحة الحقيقية (itch.io) للتحميل/الشراء الفعلي
...
app.use(async (req, res, next) => {
  try {
    ...
    res.status(503).json({ error: 'تعذر الاتصال بقاعدة البيانات: ' + e.message });
  }
});
...
/* ── SEED ألعاب حقيقية من itch.io (مجانية ومدفوعة) ─────
   يعتمد على خلاصات RSS الرسمية والمستقرة من itch.io.
   كل لعبة تُحفظ مع storeUrl يوجّه لصفحتها الحقيقية على itch.io
   حيث يتم عرض السعر الفعلي والتحميل/الشراء الحقيقي. ──────── */
...
function parseItchRSS(xml) {
  ...
  while ((m = itemRe.exec(xml))) {
    ...
    const rawDesc = clean((block.match(/<description>([\s\S]*?)<\/description>/) || [])[1]);
    ...
    items.push({
      ...
      desc: stripHtml(rawDesc).slice(0, 400) || `${title} — لعبة من itch.io`,
    });
  }
  return items;
}
app.get('/api/setup/seed-itch', wrap(async (req, res) => {
  const { key, tag = '', platform = 'pc' } = req.query;
  if (!key || key !== SEED_KEY) return res.status(403).json({ error: 'مفتاح غير صحيح' });

  const feedUrl = tag
    ? `https://itch.io/games/tag-${encodeURIComponent(tag)}.xml`
    : `https://itch.io/games.xml`;

  const r = await fetch(feedUrl, { headers: { 'User-Agent': 'AMARIX007-bot' } });
  if (!r.ok) return res.status(502).json({ error: 'تعذر الوصول لخلاصة itch.io' });
  const xml = await r.text();
  const items = parseItchRSS(xml);

  let inserted = 0, skipped = 0;
  for (const it of items) {
    const exists = await Game.findOne({ title: it.title, source: 'itch' });
    if (exists) { skipped++; continue; }
    try {
      await Game.create({
        title: it.title,
  ...
  ok(res, { inserted, skipped, total: items.length });
}));

/* ============================================================
   ...
   · نافذة تفاصيل اللعبة (Quick View) · دعم روابط itch.io الخارجية
   ============================================================ */
const AMX = (() => {
'use strict';
...
function cardHTML(g){
  ...
  const isExternal = !!g.storeUrl;
  const ctaLabel = isExternal ? '🔗 عرض على itch.io' : (f===0?'الحصول مجاناً':'إضافة للسلة');
  const ctaAttrs = isExternal
    ? `data-ext="${esc(g.storeUrl)}"`
    : `data-add="${g._id}"`;
  return `<article class="card reveal" data-slug="${esc(g.slug)}">
    <div class="card-media">
      ...
      <div class="card-badges">
        ...
        ${isExternal ? `<span class="pill">itch.io</span>` : ''}
    ...
    <div class="card-body">
      ...
      <div class="card-foot">
        <div class="card-price">
          ${isExternal ? `<span class="muted" style="font-size:.85rem">السعر على itch.io</span>` : `
          ${g.discount ? `<span class="old">$${(g.price||0).toFixed(2)}</span>` : ''}
          <span class="new ${f===0?'free':''}">${money(f)}</span>`}
        </div>
        ${g.rating ? `<span class="rating">★ ${(g.rating||0).toFixed(1)}</span>` : ''}
        <button class="add" ${ctaAttrs} aria-label="${ctaLabel}">${isExternal ? '🔗' : (f===0?'⭳':'＋')}</button>
      </div>
    </div>
    <div class="card-cta"><button class="btn btn-primary btn-sm btn-block" ${ctaAttrs}>${ctaLabel}</button></div>
  </article>`;
}
const genreName = k => {
...
function bindAdds(root=document){
  ...
  $$('[data-ext]', root).forEach(b => b.onclick = e => {
    e.preventDefault(); e.stopPropagation();
    window.open(b.dataset.ext, '_blank', 'noopener');
  });
}
...
function openDetailsModal(slug){
  ...
  api.getGame(slug).then(g => {
    ...
    const genres = Array.isArray(g.genres) ? g.genres : [];
    const isExternal = !!g.storeUrl;
    const ctaLabel = isExternal ? '🔗 فتح صفحة اللعبة على itch.io' : (f===0?'الحصول مجاناً':'إضافة للسلة');
    const ctaAttrs = isExternal ? `data-ext="${esc(g.storeUrl)}"` : `data-add="${g._id}"`;
    m.innerHTML = `
      ...
      <div style="opacity:.7;margin-bottom:.7rem;font-size:.9rem">
        ${genres.map(genreName).join(' · ')} ${genres.length ? '·' : ''}
        ${g.rating ? `★ ${(g.rating||0).toFixed(1)}` : ''}
        ${g.platform ? ' · ' + (g.platform === 'pc' ? '🖥 PC' : '📱 Android') : ''}
        ${isExternal ? ' · <span style="opacity:.8">مصدر: itch.io</span>' : ''}
      </div>
      ...
      <div style="display:flex;align-items:center;gap:.7rem;margin:1rem 0">
        ${isExternal ? `<span class="muted">السعر الحقيقي والتحميل على itch.io</span>` : `
      ...
      <button class="btn btn-primary btn-block" ${ctaAttrs}>${ctaLabel}</button>`;
...
document.addEventListener('click', e => {
  ...
  if (card && !e.target.closest('[data-add]') && !e.target.closest('[data-ext]')) openDetailsModal(card.dataset.slug);
});
...
async function heroCarousel(sel){
  ...
  box.innerHTML = `
    ${games.map((g,i) => {
      const isExternal = !!g.storeUrl;
      const ctaAttrs = isExternal ? `data-ext="${esc(g.storeUrl)}"` : `data-add="${g._id}"`;
      const ctaLabel = isExternal ? '🔗 عرض على itch.io' : '🛒 Buy Now';
      return `
      <div class="slide ${i===0?'on':''}">
        ...
        <div class="slide-inner"><div class="wrap"><div class="slide-copy">
          ...
          <div class="slide-meta">
            ...
            ${g.rating ? `<span class="tag">★ ${(g.rating||0).toFixed(1)}</span>` : ''}
          </div>
          <div class="slide-meta price-row">
            ${isExternal ? `<span class="price-new">السعر على itch.io</span>` : `
          ...
          <div class="hero-ctas">
            <button class="btn btn-primary btn-lg" ${ctaAttrs}>${ctaLabel}</button>
  ...
  const run = async () => {
    ...
    pop.innerHTML = games.length ? games.map(g => `
      <a class="sp-row" href="store.html?search=${encodeURIComponent(g.title)}">
        ...
        <div><b>${esc(g.title)}</b><br><span>${g.platform==='pc'?'PC':'Android'} · ${g.storeUrl ? 'itch.io' : money(final(g))}</span></div>
